...
- 动作型 tools 对普通 app 采用“非侵入优先，物理指针路径显式 opt-in”策略：
  ...
  - `scroll.pages` 对齐官方 `1.0.755` 的 `number` schema，支持小数页数；目标暴露可设置的 AX scrollbar 时优先按约 0.2 个 range/页调整其 `AXValue`，保证上下方向对称且可观察；否则再尝试 `AXScroll*ByPage`，动作返回失败或位置未改变时才用 `CGEvent.postToPid` 向目标进程定向发送 scroll event。全局 HID 滚轮仍只在显式安全门开启时使用。
