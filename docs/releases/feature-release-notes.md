...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-08-08 | V1.2 可控环境可靠性 | Agent 在弹窗、异步变化、多窗口、跨应用、视觉回退和权限拒绝场景中能更稳定地验证目标、有限恢复并安全停止，同时减少无意义读取和截图开销。 | 发布 `1.2.0`：增加状态版本、窗口指纹、旧索引拒绝、结构树优先/截图回退、安全边界和 Codex×GPT / Claude Code×DeepSeek 组合约束；12 场景各 5 次正式配对中 OCU 任务 60/60、方法 58/60、错误目标 0、安全违规 0，自动评分 96.6/100。 |
